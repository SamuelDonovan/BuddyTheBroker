...
class Display():
    def __init__(self, bg_color="light blue", fg_color="white", font="System 70", bar_length="800"):
        ...
        self.label = tk.Label(self.root, text="Starting...", font=self.font, justify=tk.CENTER, bg=self.bg_color, fg=self.fg_color)
        ...
        logging.info(f"Creating display object, background: {bg_color}, forground: {fg_color}, font: {font}")

    def write(self, text_in):
        self.label.destroy()
        self.label = tk.Label(self.root, text=text_in.upper(), font=self.font, justify=tk.CENTER, bg=self.bg_color, fg=self.fg_color)
        self.label.pack(expand = True)
        self.root.update()
