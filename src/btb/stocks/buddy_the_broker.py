...
class BuddyTheBroker():
    def main():
        ...
        if rh.login(username=robinhood_username, password=robinhood_password, qr_code=robinhood_qr_code):
            logging.info(f"Signed in to robinhood account {robinhood_username}.")
        else:
            logging.error(f"Failed to sign in to robinhood account {robinhood_username}. Please verify account username, password, and qr_code are set with the keyring module.")
            raise Exception
